...
export default function HeroVisual({ mouse = { x: 0, y: 0 } }) {
  ...
  return (
    <div className="hero-stock-bg" ref={containerRef} aria-hidden="true">
      ...
      {/* Floating Stock Chips Layer — decorative, no price data */}
      <div className="hero-stock-chips">
        {[
          { id: 'googl', symbol: 'GOOGL', name: 'Alphabet', x: 14, y: 16, depth: 0.6 },
          { id: 'aapl', symbol: 'AAPL', name: 'Apple', x: 86, y: 14, depth: 0.85 },
          { id: 'eur', symbol: 'EUR/USD', name: 'Forex', x: 10, y: 52, depth: 0.45 },
          { id: 'nvda', symbol: 'NVDA', name: 'NVIDIA', x: 88, y: 54, depth: 0.75 },
          { id: 'gbp', symbol: 'GBP/USD', name: 'Forex', x: 18, y: 82, depth: 0.5 },
          { id: 'tsla', symbol: 'TSLA', name: 'Tesla', x: 82, y: 84, depth: 0.4 },
        ].map((chip) => {
          ...
          return (
            <div
              key={chip.id}
              className="stock-chip stock-chip--up"
              style={{
                left: `${chip.x}%`,
                top: `${chip.y}%`,
            ...
            >
              ...
              <div className="stock-chip__price" style={{ fontSize: '0.65rem', opacity: 0.6 }}>{chip.name}</div>
            </div>
          )
        })}
      </div>
